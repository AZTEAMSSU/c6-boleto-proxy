  ...
  if (req.method === "POST" && req.url === "/boleto") {
    ...
    req.on("end", async () => {
      try {
        ...
        const boletoBody = { external_reference: externalRef, amount: parseFloat(amount), due_date: dueDate, payer: { name: payerName, document: payerDocument.replace(/\D/g, ""), address: { street: payerStreet || "", number: payerNumber || "", city: payerCity || "", state: payerState || "", zip_code: (payerZip || "").replace(/\D/g, "") } } };
